...
const AllProductsPageTemplate = ({
  data: { allContentfulProject: projects },
  pageContext,
}) => {
  const localizedData = {
    en: { path: "/en/projects/", title: "Projects" },
    es: { path: "/es/proyectos/", title: "Proyectos" },
  }
  ...
  return (
    <Layout {...pageContext}>
      ...
      <Wrapper>
        <h1>{title}</h1>
        <Grid>
          {projects.edges.map(({ node: page }) => (
            <ItemImage
              key={page.id}
              title={page.title}
              slug={page.slug}
              image={page.images[0].fluid}
